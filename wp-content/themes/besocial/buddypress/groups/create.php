	...
	<form action="<?php bp_group_creation_form_action(); ?>" method="post" id="create-group-form" class="standard-form" enctype="multipart/form-data">
		...
		<div class="item-body" id="group-create-body">
			...
			<?php if ( bp_is_group_creation_step( 'group-details' ) ) : ?>
				...
				<div>
					<label for="group-name"><?php esc_html_e( 'Group Name (required)', 'besocial' ); ?></label>
					<input type="text" name="group-name" id="group-name" aria-required="true" value="<?php bp_new_group_name(); ?>" />
				</div>

				<div>
					<label for="group-desc"><?php esc_html_e( 'Group Description (required)', 'besocial' ); ?></label>
			...
			<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
				...
				<fieldset class="group-create-privacy">
					...
					<div class="radio">
						...
						<ul id="public-group-description">
							<li><?php esc_html_e( 'Any site member can join this group.', 'besocial' ); ?></li>
							<li><?php esc_html_e( 'This group will be listed in the groups directory and in search results.', 'besocial' ); ?></li>
							<li><?php esc_html_e( 'Group content and activity will be visible to any site member.', 'besocial' ); ?></li>
						</ul>
						...
						<ul id="private-group-description">
							<li><?php esc_html_e( 'Only users who request membership and are accepted can join the group.', 'besocial' ); ?></li>
							<li><?php esc_html_e( 'This group will be listed in the groups directory and in search results.', 'besocial' ); ?></li>
							<li><?php esc_html_e( 'Group content and activity will only be visible to members of the group.', 'besocial' ); ?></li>
						</ul>
				...
				<fieldset class="group-create-invitations">
					...
					<p><?php esc_html_e( 'Which members of this group are allowed to invite others?', 'besocial' ); ?></p>

					<div class="radio">

						<label for="group-invite-status-members"><input type="radio" name="group-invite-status" id="group-invite-status-members" value="members"<?php bp_group_show_invite_status_setting( 'members' ); ?> /> <?php esc_html_e( 'All group members', 'besocial' ); ?></label>
				...
				<?php if ( bp_is_active( 'forums' ) ) : ?>
					...
					<h4><?php esc_html_e( 'Group Forums', 'besocial' ); ?></h4>

					<?php if ( bp_forums_is_installed_correctly() ) : ?>

						<p><?php esc_html_e( 'Should this group have a forum?', 'besocial' ); ?></p>

						<div class="checkbox">
							<label for="group-show-forum"><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php checked( bp_get_new_group_enable_forum(), true, true ); ?> /> <?php esc_html_e( 'Enable discussion forum', 'besocial' ); ?></label>
						</div>
					<?php elseif ( is_super_admin() ) : ?>

						<p><?php printf( __( '<strong>Attention Site Admin:</strong> Group forums require the <a href="%s">correct setup and configuration</a> of a bbPress installation.', 'besocial' ), bp_core_do_network_admin() ? network_admin_url( 'settings.php?page=bb-forums-setup' ) :  admin_url( 'admin.php?page=bb-forums-setup' ) ); ?></p>
			...
			<?php if ( bp_is_group_creation_step( 'group-avatar' ) ) : ?>
				...
				<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
					...
					<div class="main-column">
						<p><?php esc_html_e( "Upload an image to use as a profile photo for this group. The image will be shown on the main group page, and in search results.", 'besocial' ); ?></p>

						<p>
							<label for="file" class="bp-screen-reader-text"><?php
								/* translators: accessibility text */
								_e( 'Select an image', 'besocial' );
							?></label>
							<input type="file" name="file" id="file" />
							<input type="submit" name="upload" id="upload" value="<?php esc_attr_e( 'Upload Image', 'besocial' ); ?>" />
							<input type="hidden" name="action" id="action" value="bp_avatar_upload" />
						</p>

						<p><?php esc_html_e( 'To skip the group profile photo upload process, hit the "Next Step" button.', 'besocial' ); ?></p>
					</div><!-- .main-column -->
				...
				<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
					...
					<h4><?php esc_html_e( 'Crop Group Profile Photo', 'besocial' ); ?></h4>

					<img src="<?php bp_avatar_to_crop(); ?>" id="avatar-to-crop" class="avatar" alt="<?php esc_attr_e( 'Profile photo to crop', 'besocial' ); ?>" />

					<div id="avatar-crop-pane">
						<img src="<?php bp_avatar_to_crop(); ?>" id="avatar-crop-preview" class="avatar" alt="<?php esc_attr_e( 'Profile photo preview', 'besocial' ); ?>" />
					</div>

					<input type="submit" name="avatar-crop-submit" id="avatar-crop-submit" value="<?php esc_attr_e( 'Crop Image', 'besocial' ); ?>" />
            ...
            <div class="besocial-invite-lists">
					<div class="besocial-invite-left">

						<div id="besocial-invite-list">
							<ul>
					...
					<div class="besocial-invite-right">

						<div id="message" class="info">
							<p><?php esc_html_e('Select people to invite from your friends list.', 'besocial' ); ?></p>
						</div>
						...
						<?php if ( bp_group_has_invites() ) : ?>
							...
							<?php while ( bp_group_invites() ) : bp_group_the_invite(); ?>
								...
								<li id="<?php bp_group_invite_item_id(); ?>">
									...
									<div class="action">
										<a class="remove" href="<?php bp_group_invite_user_remove_invite_url(); ?>" id="<?php bp_group_invite_item_id(); ?>"><?php esc_html_e( 'Remove Invite', 'besocial' ); ?></a>
									</div>
								</li>
				...
				<?php else : ?>
					...
					<div id="message" class="info">
						<p><?php esc_html_e( 'Once you have built up friend connections you will be able to invite others to your group.', 'besocial' ); ?></p>
					</div>
			...
			<?php if ( 'crop-image' != bp_get_avatar_admin_step() ) : ?>
				...
				<div class="submit" id="previous-next">
					...
					<?php if ( !bp_is_first_group_creation_step() ) : ?>
						...
						<input type="button" value="<?php esc_attr_e( 'Back to Previous Step', 'besocial' ); ?>" id="group-creation-previous" name="previous" onclick="location.href='<?php bp_group_creation_previous_link(); ?>'" />
					...
					<?php if ( !bp_is_last_group_creation_step() && !bp_is_first_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Next Step', 'besocial' ); ?>" id="group-creation-next" name="save" />
					...
					<?php if ( bp_is_first_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Create Group and Continue', 'besocial' ); ?>" id="group-creation-create" name="save" />
					...
					<?php if ( bp_is_last_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Finish', 'besocial' ); ?>" id="group-creation-finish" name="save" />
