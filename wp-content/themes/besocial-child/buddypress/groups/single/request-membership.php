...
<?php if ( !bp_group_has_requested_membership() ) : ?>
	<h2 class="bp-screen-reader-text"><?php
		...
		_e( 'Request form', 'besocial' );
	?></h2>

	<p><?php printf( __( "You are requesting to become a affiliate of the group '%s'.", 'besocial' ), bp_get_group_name( false ) ); ?></p>

	<form action="<?php bp_group_form_action('request-membership' ); ?>" method="post" name="request-membership-form" id="request-membership-form" class="standard-form">
		<label for="group-request-membership-comments"><?php esc_html_e( 'Comments (optional)', 'besocial' ); ?></label>
		...
		<p><input type="submit" name="group-request-send" id="group-request-send" value="<?php esc_attr_e( 'Send Request', 'besocial' ); ?>" />
