		...
		<table id="message-threads" class="messages-notices">
			...
			<thead>
				<tr>
					...
					?></label></th>
					<th scope="col" class="thread-from"><?php esc_html_e( 'From', 'besocial' ); ?></th>
					<th scope="col" class="thread-info"><?php esc_html_e( 'Subject', 'besocial' ); ?></th>
					...
					<th scope="col" class="thread-options"><?php esc_html_e( 'Actions', 'besocial' ); ?></th>
				</tr>
			</thead>
					...
					<tr id="m-<?php bp_message_thread_id(); ?>" class="<?php bp_message_css_class(); ?><?php if ( bp_message_thread_has_unread() ) : ?> unread<?php else: ?> read<?php endif; ?>">
						<td class="bulk-select-check">
							<label for="bp-message-thread-<?php bp_message_thread_id(); ?>"><input type="checkbox" name="message_ids[]" id="bp-message-thread-<?php bp_message_thread_id(); ?>" class="message-check" value="<?php bp_message_thread_id(); ?>" /><span class="bp-screen-reader-text"><?php
								/* translators: accessibility text */
								_e( 'Select this message', 'besocial' );
							?></span></label>
						</td>
						...
						<?php if ( 'sentbox' != bp_current_action() ) : ?>
							<td class="thread-from">
								...
								<span class="from"><?php esc_html_e( 'From:', 'besocial' ); ?></span> <?php bp_message_thread_from(); ?>
						...
						<?php else: ?>
							<td class="thread-from">
								<?php bp_message_thread_avatar( array( 'width' => 25, 'height' => 25 ) ); ?>
								<span class="to"><?php esc_html_e( 'To:', 'besocial' ); ?></span> <?php bp_message_thread_to(); ?>
								<?php bp_message_thread_total_and_unread_count(); ?>
								<span class="activity"><?php bp_message_thread_last_post_date(); ?></span>
							</td>
						<?php endif; ?>
						...
						<td class="thread-info">
							<p><a href="<?php bp_message_thread_view_link(); ?>" title="<?php esc_attr_e( "View Message", 'besocial' ); ?>"><?php bp_message_thread_subject(); ?></a></p>
						...
						<td class="thread-options">
							<?php if ( bp_message_thread_has_unread() ) : ?>
								<a class="read" href="<?php bp_the_message_thread_mark_read_url();?>"><?php esc_html_e( 'Read', 'besocial' ); ?></a>
							<?php else : ?>
								<a class="unread" href="<?php bp_the_message_thread_mark_unread_url();?>"><?php esc_html_e( 'Unread', 'besocial' ); ?></a>
							<?php endif; ?>
							 |
							<a class="delete" href="<?php bp_message_thread_delete_link(); ?>"><?php esc_html_e( 'Delete', 'besocial' ); ?></a>
...
<?php else: ?>
	...
	<div id="message" class="info">
		<p><?php esc_html_e( 'Sorry, no messages were found.', 'besocial' ); ?></p>
	</div>
