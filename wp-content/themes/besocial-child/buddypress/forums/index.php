...
<div class="buddypress-holder">
	...
	<form  method="post" id="forums-search-form" class="dir-form">
	...
	<form  method="post" id="forums-directory-form" class="dir-form">

		<div class="item-list-tabs" aria-label="<?php esc_attr_e( 'Forums directory main navigation', 'besocial' ); ?>" role="navigation">
			<ul>
				<li class="selected" id="forums-all"><a href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_forums_root_slug() ); ?>"><?php printf( __( 'All Topics %s', 'besocial' ), '<span>' . bp_get_forum_topic_count() . '</span>' ); ?></a></li>

				<?php if ( is_user_logged_in() && bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) ) : ?>

					<li id="forums-personal"><a href="<?php echo trailingslashit( bp_loggedin_user_domain() . bp_get_forums_slug() . '/topics' ); ?>"><?php printf( __( 'My Topics %s', 'besocial' ), '<span>' . bp_get_forum_topic_count_for_user( bp_loggedin_user_id() ) . '</span>' ); ?></a></li>
		...
		<div class="item-list-tabs" id="subnav" aria-label="<?php esc_attr_e( 'Forums secondary navigation', 'besocial' ); ?>" role="navigation">
			<ul>
		...
		<?php if ( is_user_logged_in() ) : ?>
			...
			<?php if ( bp_is_active( 'groups' ) && bp_has_groups( 'user_id=' . bp_loggedin_user_id() . '&type=alphabetical&max=100&per_page=100' ) ) : ?>
				...
				<form  method="post" id="forum-topic-form" class="standard-form">
					...
					<a name="post-new"></a>
					<h5><?php esc_html_e( 'Create New Topic:', 'besocial' ); ?></h5>
					...
					<label for="topic_group_id"><?php esc_html_e( 'Post In Group Forum:', 'besocial' ); ?></label>
					<select id="topic_group_id" name="topic_group_id">

						<option value=""><?php /* translators: no option picked in select box */ _e( '----', 'besocial' ); ?></option>
					...
					<div class="submit">
						...
						<input type="button" name="submit_topic_cancel" id="submit_topic_cancel" value="<?php esc_attr_e( 'Cancel', 'besocial' ); ?>" />
					</div>
			...
			<?php elseif ( bp_is_active( 'groups' ) ) : ?>
				...
				<div id="message" class="info">
					...
					<p><?php printf( __( "You are not a affiliate of any groups so you don't have any group forums you can post in. To start posting, first find a group that matches the topic subject you'd like to start. If this group does not exist, why not <a href='%s'>create a new group</a>? Once you have joined or created the group you can post your topic in that group's forum.", 'besocial' ), trailingslashit( bp_get_groups_directory_permalink() . 'create' ) ); ?></p>
