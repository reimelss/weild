	...
	function dislike_ajax(id){
			...
			dataType: "json",
			data:{
				action:'besocial_system_dislike_button',
				post_id:id,
				nonce: besocial_ajax_var.nonce
			},
			success: function(response){
			...
			complete:function(){
				jQuery(document.body).one('click.besocialdislike','.besocial-p-dislike',dislike);
			}
		});
	}
	...
	if(Modernizr.touchevents){
		jQuery(document.body).on('mouseleave touchmove click', '.besocial-p-like', function( event ) {
			if(jQuery(this).hasClass('besocial-p-like-active')){
				var color = jQuery('.besocial-p-dislike').css('color');
				jQuery(this).css('color',color);
			}else{
				jQuery(this).removeAttr('style');
			};
		});
		jQuery(document.body).on('mouseleave touchmove click', '.besocial-p-dislike', function( event ) {
			if(jQuery(this).hasClass('besocial-p-dislike-active')){
				var color = jQuery('.besocial-p-like').css('color');
				jQuery(this).css('color',color);
			}else{
				jQuery(this).removeAttr('style');
			};
